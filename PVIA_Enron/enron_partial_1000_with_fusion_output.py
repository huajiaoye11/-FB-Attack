...
def test_data_top(edb_filename, partial_filename, frac1, frac2, word_size=500):
    ...
    for ed_key, ed_value in tqdm.tqdm(edb_keyword.items()):
        count_in = 0
        query_name = "query_" + str(ed_key)#
        pvia_scores[query_name] = {}#
        ...
        for par_key, par_value in partial_keyword.items():
            ...
            if res_percent >= (frac1 + frac2) - 1:
                count_in += 1
                pvia_scores[query_name][str(par_key)] = float(res_percent)#
    ...
    output_path = fusion_input_dir / "pvia_candidates.json"#

    with open(output_path, "w", encoding="utf-8") as f:#
        json.dump(pvia_scores, f, ensure_ascii=False, indent=2)#

    print("PVIA candidates saved to:", output_path)#
