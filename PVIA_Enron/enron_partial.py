...
EDB_DIR = BASE_DIR / "edb"
PARTIAL_DIR = BASE_DIR / "partial"
RESULTS_DIR = BASE_DIR / "results"

WORD_SIZE = 1000
SUPPORTED_WORD_SIZES = [500, 1000, 2000, 3000, 5000]
...
if __name__ == "__main__":
    if WORD_SIZE not in SUPPORTED_WORD_SIZES:
        raise ValueError(
            f"WORD_SIZE={WORD_SIZE} not supported. "
            f"Choose from {SUPPORTED_WORD_SIZES}"
        )

    result_filename = f"enron_partial_{WORD_SIZE}.txt"
    ...
    for i in frac1:
        for j in frac2:
            filename1 = str(EDB_DIR / (str(i) + ".json"))
            filename2 = str(PARTIAL_DIR / (str(j) + ".json"))
            count_uni = test_data_top(filename1, filename2, i, j, WORD_SIZE)

            result_line = f"{i} {j} containing unique result {count_uni}"
            save_txt_result_lines(result_path, [result_line])
