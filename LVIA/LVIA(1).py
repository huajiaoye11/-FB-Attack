...
def static_enron_data_info(dataset_name, nkw, n_month, alpha=0.5):
    ...
    for i in range(i_count + 1):
        ...
        known_index = set(
            sample(
                set(range(len(dataset_by_month[i]))) - target_index,
                int(sampleSetSize * alpha / 0.5)
            )
        )
        ...
        if not flag:
            ...
            for q in keyword2query[target_keyword]:
                ...
                for interval in range(i_count + 1):
                    cur_dist += np.abs(
                        keyword_split_attack[interval][target_keyword]
                        - keyword_split_known[interval][q]
                    )
...
if __name__ == "__main__":
    ...
    # --------------------------------------------------------
    # 原本固定用 Pool(32)
    # 但如果电脑不是服务器，32 个进程可能反而更慢
    #
    # 这里最多使用 8 个进程
    worker_num = min(os.cpu_count(), 8)
    ...
    # 9. 多进程并行运行
    # --------------------------------------------------------
    # starmap 会把 tasks 里面的每一组参数传给 static_enron_data_info
    with Pool(worker_num) as pool:
        results = pool.starmap(static_enron_data_info, tasks)
        ...
        for nkw in nkw_list:
            for n_month in query_month:
                for alpha in alphas:
                    ...
                    acc_mean = np.mean(list_res)
                    acc_std = np.std(list_res)
