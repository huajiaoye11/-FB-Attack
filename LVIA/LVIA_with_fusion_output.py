...
def static_enron_data_info(dataset_name, nkw, n_month, alpha=0.5):
    ...
    for query_id, cand_counter in enumerate(counter):
        if query_id not in id_to_word:
            continue

        true_keyword = id_to_word[query_id]
        query_name = "query_" + true_keyword

        lvia_scores[query_name] = {}

        if len(cand_counter) == 0:
            continue

        max_count = max(cand_counter.values())

        for candidate_keyword_id, count in cand_counter.items():
            if candidate_keyword_id not in id_to_word:
                continue

            candidate_keyword = id_to_word[candidate_keyword_id]
            score = count / max_count if max_count > 0 else 0.0

            lvia_scores[query_name][candidate_keyword] = float(score)

    fusion_input_dir = Path("fusion") / "input"
    fusion_input_dir.mkdir(parents=True, exist_ok=True)

    output_path = fusion_input_dir / "lvia_candidates.json"

    with open(output_path, "w", encoding="utf-8") as f:
        json.dump(lvia_scores, f, ensure_ascii=False, indent=2)

    print("LVIA candidates saved to:", output_path)
