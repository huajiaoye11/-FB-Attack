...
def static_enron_data_info(dataset_name, nkw, trend_norm, n_month, query_number, delete_rate):
    ...
    for query_id, candidate_set in new_candidate.items():
        ...
        if query_id in ttc and len(ttc[query_id]) > 0:
            ...
            true_keyword_id = query_id

        if true_keyword_id not in id_to_word:
            continue

        true_keyword = id_to_word[true_keyword_id]
        query_name = "query_" + true_keyword

        ground_truth[query_name] = true_keyword
        fma_scores[query_name] = {}

        for candidate_keyword_id in candidate_set:
    ...
    with open(truth_output_path, "w", encoding="utf-8") as f:
